...
public class PlayerMovementScript : MonoBehaviour {
	...
	*/
	...
	bool ignoreFirstPortalCollision;
	bool ignoreSecondPortalCollision;

	void OnTriggerEnter(Collider collider)
	{
		if(collider.tag == "FirstPortal")
		{
			if(!ignoreFirstPortalCollision)
			{
				ignoreSecondPortalCollision = true;	
				transform.position = GameManager.Instance.secondPortal.transform.position;
				if(GameManager.Instance.secondPortal.transform.rotation == GameManager.Instance.firstPortal.transform.rotation)
				{
					//TODO: If rotation is same then calculate current player's rotation and adjust accordingly
					transform.rotation = GameManager.Instance.secondPortal.transform.rotation;
		...
		if(collider.tag == "SecondPortal")
		{
			if(!ignoreSecondPortalCollision)
			{
				ignoreFirstPortalCollision = true;
				transform.position = GameManager.Instance.firstPortal.transform.position;
				if(GameManager.Instance.secondPortal.transform.rotation == GameManager.Instance.firstPortal.transform.rotation)
				{
					//TODO: If rotation is same then calculate current player's rotation and adjust accordingly
					transform.rotation = GameManager.Instance.secondPortal.transform.rotation;
				...
				{
					transform.rotation = GameManager.Instance.firstPortal.transform.rotation;
				}
			}
		}
	}
	void OnTriggerExit(Collider collider)
	{
		if(collider.tag == "FirstPortal")
		{
			ignoreFirstPortalCollision = false;
		}
		if(collider.tag == "SecondPortal")
		{
			ignoreSecondPortalCollision = false;
		}
	}
